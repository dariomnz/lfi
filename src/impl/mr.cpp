...
lfi_mr_key LFI::mr_reg(void *addr, size_t size) {
    ...
    if (peer_ep.initialized()) {
        ...
        if (ret < 0) {
            ...
            return {-LFI_LIBFABRIC_ERROR, -LFI_LIBFABRIC_ERROR};
        }
        ...
        key.peer_key = ret;
        debug_info("fi_mr_reg peer key " << key.peer_key);
    }
    ...
    if (shm_ep.initialized() && shm_ep.domain != peer_ep.domain) {
        ...
        key.shm_key = ret;
        debug_info("fi_mr_reg shm key " << key.shm_key);
    } else {
        key.shm_key = key.peer_key;
    }

    m_mrs.emplace(key, std::move(mr));

    debug_info("(" << addr << ", " << size << ")=" << key.shm_key << "-" << key.peer_key << " >> End");
    return key;
}

int LFI::mr_unreg(lfi_mr_key key) {
    debug_info("(" << key.shm_key << "-" << key.peer_key << ") >> Begin");
    std::unique_lock lock(m_mr_mutex);
    auto it = m_mrs.find(key);
    if (it == m_mrs.end()) {
        debug_error("Memory region not found: " << key.shm_key << "-" << key.peer_key);
    ...
    m_mrs.erase(it);
    debug_info("(" << key.shm_key << "-" << key.peer_key << ") >> End");
    return LFI_SUCCESS;
}
}  // namespace LFI
