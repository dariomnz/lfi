...
int main(int argc, char *argv[]) {
    ...
    do {
        ...
        if (!sqe) break;
        io_uring_prep_read(sqe, fd, iovecs[i].iov_base, iovecs[i].iov_len, offset);
    ...
    for (i = 0; i < pending; i++) {
        ...
        std::cout << std::string_view((char *)iovecs[i].iov_base, iovecs[i].iov_len) << std::endl;
        done++;
        ret = 0;
    ...
    for (i = 0; i < QD; i++) free(iovecs[i].iov_base);
    free(iovecs);
    return 0;
}