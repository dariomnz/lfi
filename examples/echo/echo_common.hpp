...
[[maybe_unused]] static void print_header() {
    ...
    if (rank == 0) {
        std::cout << std::left << std::setw(6) << "Msg"
                  ...
                  << " | " << std::setw(9) << "Bw (Mbps)"
                  << " | " << std::setw(11) << "Msg (msg/s)" << std::endl;
    }
}
...
[[maybe_unused]] static void print_test(bw_test &test) {
    ...
    if (rank == 0) {
        std::cout << std::left;
        std::cout << std::setw(6) << bytes_to_str(test.test_size) << " | ";
        std::cout << std::setw(5) << test.test_count << " | ";
        std::cout << std::setw(7) << bytes_to_str(r_size) << " | ";
        std::cout << std::setw(8) << std::fixed << std::setprecision(5) << sec << " | ";
        std::cout << std::setw(8) << std::fixed << std::setprecision(5)
                  << (double)test.nanosec / 1'000'000.0 / (double)test.test_count << " | ";
        std::cout << std::setw(10) << std::fixed << std::setprecision(0) << bw << " | ";
        std::cout << std::setw(9) << std::fixed << std::setprecision(0) << (((bw * 1024 * 1024) * 8) / 1'000'000)
                  << " | ";
        std::cout << std::setw(11) << std::fixed << std::setprecision(0)
                  << (double)test.test_count / ((double)test.nanosec / 1'000'000'000.0);
        std::cout << std::endl;
    }
}

template <typename T, typename R>
[[maybe_unused]] static void busy_loop(std::chrono::duration<T, R> time) {
